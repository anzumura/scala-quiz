...
object FileUtils extends ThrowsDomainException:
  ...
  def checkExists(path: Path, extension: Option[String] = None): Path =
    if Files.exists(path) then path
    else
      extension match
  ...
  def resolve(dir: Path, file: Path, extension: Option[String] = None): Path =
    if !Files.isDirectory(dir) then error(s"'$dir' is not a directory")
    if file.isAbsolute then error(s"'$file' is an absolute path")
    val result = checkExists(dir.resolve(file), extension)
    if !Files.isRegularFile(result) then error(s"'$result' is not a regular file")
    result
  ...
  def getFiles(dir: Path): LazyList[Path] = Files.list(dir).toScala(LazyList)
    .filter(Files.isRegularFile(_)).sorted

  def getDirectories(dir: Path): LazyList[Path] = Files.list(dir).toScala(LazyList)
    .filter(Files.isDirectory(_)).sorted
