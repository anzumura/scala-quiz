...
class RadicalData(dir: Path) extends ThrowsDomainException:
  ...
  def findByNumber(i: Int): Radical =
    ...
    if data.nonEmpty && i > 0 && i <= MaxRadical then radicals(i - 1)
  ...
  protected def verifyDataSize(x: Int): Unit =
    if x != MaxRadical then domainError(s"loaded $x, but expected " + s"$MaxRadical")

  private def verifyRadicalNumber(x: Int): Unit =
    if x == 0 then error("Radical number can't be zero")
    if Option(radicals(x - 1)).nonEmpty then error(s"duplicate Radical number '$x'")
