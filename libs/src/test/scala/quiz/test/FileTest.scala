...
trait FileTest extends BaseTest with BeforeAndAfterAll:
  ...
  def delete(p: Path): Unit = Try(Files.delete(p)).failed.foreach { e =>
    println(s"DELETE FAILED: $e")
    System.exit(1)
  }

  def deleteIfExists(p: Path): Unit = Try(Files.deleteIfExists(p)).failed.foreach { e =>
    println(s"DELETE FAILED: $e")
    System.exit(1)
  }

  def clearDirectory(d: Path): Unit = Files.walk(d).toScala(LazyList).reverse.filter(_ != d)
    .foreach(delete)
  ...
  def writeTestFile(lines: String*): Path =
    if lines.isEmpty then Files.createFile(testFile) else Files.write(testFile, lines.asJava)
